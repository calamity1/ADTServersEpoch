...
[[_position select 0, _position select 1, 0],4,1,"Random",5,"","TK_INS_Soldier_AT_EP1","Random",true] call spawn_group;
[[_position select 0, _position select 1, 0],4,1,"Random",5,"","TK_INS_Soldier_AT_EP1","Random",true] call spawn_group;
[[_position select 0, _position select 1, 0],4,1,"Random",5,"","TK_INS_Soldier_AT_EP1","Random",true] call spawn_group;
[[_position select 0, _position select 1, 0],4,1,"Random",5,"","TK_INS_Soldier_AT_EP1","Random",true] call spawn_group;
...
[[[(_position select 0) + 30, (_position select 1) - 30, 8]],"M2StaticMG",0.8,"TK_INS_Soldier_AT_EP1",1,2,"","Random",true] call spawn_static;
[[[(_position select 0) + 7, (_position select 1) - 7, 8]],"M2StaticMG",0.8,"TK_INS_Soldier_AT_EP1",1,2,"","Random",true] call spawn_static;
[[[(_position select 0) - 16, (_position select 1) + 16, 8]],"M2StaticMG",0.8,"TK_INS_Soldier_AT_EP1",1,2,"","Random",true] call spawn_static;
 
[_position,"Mayors Mansion"] execVM "\z\addons\dayz_server\WAI\missions\compile\markers.sqf";
[nil,nil,rTitleText,"Бандит дезертирует,и его задача вытеснить претендовать на черном рынке оружия! Убей его!", "PLAIN",10] call RE;
...
if (_playerPresent) then {
	...
	[nil,nil,rTitleText,"Главарь Бандитов был убит !!!", "PLAIN",10] call RE;
} else {
	...
	[nil,nil,rTitleText,"Выжившие не успели убить главаря банды!", "PLAIN",10] call RE;
};
