...
if ( AGN_safeZoneMessages ) then { systemChat ( "Загрузка торговой зоны!" ); };
...
while {true} do {
	...
	if ( AGN_safeZoneMessages ) then { systemChat ("Вы в безопасной зоне - GodMod включен"); };
	_thePlayer = player;
	...
	if ( AGN_safeZone_Players_DisableWeaponFiring ) then
	{
		_EH_Fired = _thePlayer addEventHandler ["Fired", {
			systemChat ("Вы не можете стрелять в безопасной зоне");
			...
			while {!canBuild} do
			{
				...
				if( !isNull (FindDisplay 106) && !_skip ) then
				{
					if ( isNull AGN_LastPlayerLookedAt ) then
					{
						...
						if ( AGN_safeZoneMessages ) then { systemChat ("Анти крысятничество в рюкзаках!"); };
						waitUntil { isNull (FindDisplay 106) };
					} else {
						if ( AGN_safeZoneMessages ) then { systemChat (format["Вы не можете открыть свое снаряжение, когда смотрите на другого игрока!"]); };
						(findDisplay 106) closeDisplay 1;
						waitUntil { isNull (FindDisplay 106) };
					};
				};
				if ( _skip && _if ) then {
					if ( AGN_safeZoneMessages ) then { systemChat ("Вы не можете лутать рюкзак выжившего в этой зоне") };
				};
			};
		};
	};
	...
	if ( AGN_safeZone_Vehicles_DisableMountedGuns ) then
	{
		while { !canBuild } do
		{
			...
			{
				if (AGN_safeZoneMessages) then { systemChat ( "[ADT-TEAM] No Firing Vehicle Guns Enabled" ); };
				_inVehicle = vehicle player;
				_inVehicleDamage = getDammage _inVehicle;
				_EH_Fired_Vehicle = _inVehicle addEventHandler ["Fired", {
					systemChat ("[ADT-TEAM] Вы не можете использовать оружие на технике в этой зоне");
					NearestObject [_this select 0,_this select 4] setPos[0,0,0];
				}];
			};
		};
	...
	if ( AGN_safeZoneMessages ) then { systemChat ("Вы покинули безопасную зону - GodMod выключен "); };
	
	if ( AGN_safeZone_Vehicles_DisableMountedGuns ) then
	{
		if ( !(isNull _inVehicle) ) then
		{
			if ( AGN_safeZoneMessages ) then { systemChat ( "Не какой стрельбы!" ); };
			_inVehicle removeEventHandler ["Fired", _EH_Fired_Vehicle];
		};
		
		if ( !(isNull _inVehicleLast) ) then
		{
			if ( AGN_safeZoneMessages ) then { systemChat ( "Ваше оружие на техники не работает!" ); };
			_inVehicleLast removeEventHandler ["Fired", _EH_Fired_Vehicle];
		};
	};
