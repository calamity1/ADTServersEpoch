...
if (_showMapMarker && !("ItemMap" in _inv)) exitWith { systemChat "Вы должны иметь карту, чтобы найти свои машины!";};
_keyColor = [];
_keyID = [];
_removedID = [];
_count = 0;
...
if (_showMapMarker) then {
	if (_count == 0) then { systemChat "Проверьте карту, чтобы найти свой автомобиль!";}
	else {systemChat "Проверьте карту, чтобы найти свой автомобиль!";};
};