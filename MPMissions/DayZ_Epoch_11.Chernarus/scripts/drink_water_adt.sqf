...
if (_canDrink) then {
	if ((floor (random 100) < _stagnant_water_rates)) then {
		...
		//cutText [(localize "STR_drink_stagnant_water"), "PLAIN DOWN"]; //fix_adt
		cutText ["Вода тёплая и на вкус как моча ослинная, но тем не менее жажда утолена!", "PLAIN DOWN"];
		...
		_display = uiNamespace getVariable 'DAYZ_GUI_display';
		(_display displayCtrl 1302) ctrlShow true;
		//cutText [(localize "STR_drink_fresh_water"), "PLAIN DOWN"]; //fix_adt
		cutText ["Вода прохладная и на вкус тухловата, но жажда это всё скрашивает", "PLAIN DOWN"];
	};
};