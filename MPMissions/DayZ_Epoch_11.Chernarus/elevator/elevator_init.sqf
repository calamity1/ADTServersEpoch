...
if (isNil "ELE_MaxRange") then { ELE_MaxRange = 500 }; // m
if (isNil "ELE_Size") then { ELE_Size = 3 }; // m
if (isNil "ELE_Speed") then { ELE_Speed = 15 }; // m/s
...
if (isNil "ELE_RequiredBuildTools") then { ELE_RequiredBuildTools = ["ItemToolbox"] }; // required tools for building an elevator and elevator stop
if (isNil "ELE_RequiredBuildItems") then { ELE_RequiredBuildItems = [["PartGeneric",4], "PartEngine"] }; // required items to build an elevator
		...
		if (typeOf _ct == ELE_PlatformClass) then {
			...
			if ([_ct] call ELE_fnc_isElevator) then {
				if (s_player_elevator_next < 0 && {[_ct] call ELE_fnc_hasNextStop}) then {
					s_player_elevator_next = player addAction ["<t color=""#ffffff"">Вкл. Лифт: след. остановка</t>", _folder+"elevator_actions.sqf", ["next",_ct], 5, false];
				};
				if (s_player_elevator_previous < 0 && {[_ct] call ELE_fnc_hasPreviousStop}) then {
					s_player_elevator_previous = player addAction ["<t color=""#ffffff"">Вкл. Лифт: пред. остановка</t>", _folder+"elevator_actions.sqf", ["previous",_ct], 5, false];
				};
				if (s_player_elevator_select < 0) then {
					s_player_elevator_select = player addAction ["<t color=""#ffffff"">Выбрать Лифт</t>", _folder+"elevator_actions.sqf", ["select",_ct], 1, false];
				};
				// debug actions
		if (s_player_elevator_id < 0 && ELE_Debug) then {
			s_player_elevator_id = player addAction ["<t color=""#ddffffff"">Показать номер Лифта</t>", _folder+"elevator_actions.sqf", ["id",_ct], 0, false];
		};
			} else {
