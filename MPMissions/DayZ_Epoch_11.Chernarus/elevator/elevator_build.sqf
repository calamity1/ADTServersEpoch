...
if (DZE_ActionInProgress) exitWith { cutText ["Обновление в процессе." , "PLAIN DOWN"]; };
...
switch (_option) do {
	...
	case "build_stop": {
		...
		if (_dist > ELE_MaxRange) exitWith { cutText [format["Остановка слишком далеко (%1 > %2)", _dist, ELE_MaxRange], "PLAIN"] };
		_id = [ELE_elevator] call ELE_fnc_getNextStopId;
		if (_id == "") exitWith { cutText ["Остановка лифта уже существует (мак. 9 остановок)", "PLAIN"] };
		if ((ELE_RequiredBuildTools call AC_fnc_hasTools) && {ELE_RequiredBuildStopItems call AC_fnc_checkAndRemoveRequirements}) then {
			...
			titleText ["Создание Лифта остановлено", "PLAIN"];
		};
	};
};
