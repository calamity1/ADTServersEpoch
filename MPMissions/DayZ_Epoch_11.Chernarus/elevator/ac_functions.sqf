...
AC_fnc_hasTools = {
	...
	_hasTools = true;
	{
		if (!(_x in _items)) exitWith {
			...
			cutText [format["Отсутствует инструмент %1", _missing] , "PLAIN DOWN"];
		};
	} forEach _tools;
	_hasTools
};
...
AC_fnc_checkRequirements = {
	...
	_hasItems = true;
	{
		...
		if (_qty < _countIn) exitWith {
			...
			cutText [format["Отсутствует %1 требуется %2", _missingQty, _textMissing], "PLAIN DOWN"];
		};
	} forEach _items;
	_hasItems
};
...
AC_fnc_removeRequirements = {
	...
	cutText [format["Отсутствует части, проверьте: %1 / %2", _removed_total, _tobe_removed_total], "PLAIN DOWN"];
	false
};
