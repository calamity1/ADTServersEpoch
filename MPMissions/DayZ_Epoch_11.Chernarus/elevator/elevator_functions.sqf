...
ELE_fnc_activateElevator = {
	...
	if (_elevator getVariable ["ElevatorActive", false]) exitWith {
		cutText ["этот лифт уже активен", "PLAIN DOWN"];
	};
	_id = _elevator getVariable ["ElevatorID", 0];
	...
	if (isNil "_nextStop") exitWith {
		cutText ["следующая остановка лифта НЕ найдена", "PLAIN DOWN"];
	};
	...
	cutText [format["переход к следующей остановке (%1, %2 поехали) ...", _nextStopId, _dist], "PLAIN DOWN"];
	...
	cutText ["лифт остановился", "PLAIN DOWN"];
};
...
ELE_fnc_callElevator = {
	...
	cutText ["лифт прибыл", "PLAIN DOWN"];
};
