if (vehicle player != player) exitWith {cutText [format["Нельзя открывать Управление группой в технике!"], "PLAIN DOWN"];};
...
while {groupManagmentActive} do {
    ...
    if (_hasInvite) then {
        ...
        _groupInviteText ctrlSetStructuredText parseText (format ["Предложение вступить в группу <br/>%1",_name]);
    } else {
    	_groupAcceptInvite ctrlShow false;
        _groupDeclineInvite ctrlShow false;
