dzgm_Icons_Clear = {
	...
	_index = 0;
    {
		...
		if (visibleMap) then {
			...
			_rdmtkr = createMarkerLocal [_name,_pos];
			_rdmtkr setMarkerTypeLocal "DestroyedVehicle";
			_rdmtkr setMarkerTextLocal format ["%1",_name];
		};
		((uiNamespace getVariable "dmt_hd") displayCtrl (46300 + _index)) ctrlShow false;
        _index = _index + 1;
    } forEach (_units);
};

dzgm_Icons_Name = {
    private ["_make_icons","_rdmtkr","_pIcons","_plist","_remove_icon","_uc","_units"];
	...
	
    {
		...
		_tag = composeText [image _picon," ",_name];
			if (((count _screen) > 1) && (!visibleMap)) then {
				...
				((uiNamespace getVariable "dmt_hd") displayCtrl (46300 + _index)) ctrlSetStructuredText _tag;
				((uiNamespace getVariable "dmt_hd") displayCtrl (46300 + _index)) ctrlSetPosition [_sx, _sy, 0.99, 0.65];
				((uiNamespace getVariable "dmt_hd") displayCtrl (46300 + _index)) ctrlSetScale _scale;
				((uiNamespace getVariable "dmt_hd") displayCtrl (46300 + _index)) ctrlSetFade ((1- _scale ) / 2);
				((uiNamespace getVariable "dmt_hd") displayCtrl (46300 + _index)) ctrlCommit 0;
				((uiNamespace getVariable "dmt_hd") displayCtrl (46300 + _index)) ctrlShow true;
			} else {
				((uiNamespace getVariable "dmt_hd") displayCtrl (46300 + _index)) ctrlShow false;
			};		
		} else {
