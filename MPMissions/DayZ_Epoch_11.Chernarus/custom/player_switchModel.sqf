...
_oldUnit = player;
_oldGroup = group player; //fixes dzgm radio scripts
...
_switchUnit = {
	...
	if ((count units _oldGroup > 1) && (!isNil "PVDZE_plr_LoginRecord")) then {[player] join _oldGroup;}; //fixes dzgm radio scripts by grom
